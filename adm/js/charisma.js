...
//var current_theme = $.session.get('current_theme') == null 
//? 'cerulean' : $.session.get('current_theme');

//    var current_theme = $.cookie('current_theme') == null 
//    ? 'cerulean' : $.cookie('current_theme');
//     
//alert($.session.get('current_theme')); 

//     
// switch_theme(current_theme);
//	
//$('#themes a[data-value="'+current_theme+'"]').find('i').addClass('icon-ok');
    $('#btSaveCliente').click(function() {
        //  $('#form_cliente').submit(function() {
//action="dao/clienteSave.php"

        var my_data = $("#form_cliente").serialize();
        
        $.post('../dao/clienteSave.php', my_data, function(data) {
            return true;
        });
    });
    ...
    $('#themes a').click(function(e) {
        ...
        $.cookie('current_theme', theme_name, {
            expires: 365
        });
        switch_theme(current_theme);
    ...
    {
        //        alert("Entrou "+theme_name);
        $('#bs-css').attr('href', 'css/bootstrap-' + theme_name + '.css');
        return;
        $.ajax({
            url: 'dao/preferencesSave.php?theme=' + theme_name,
            success: function(msg) {
                alert("Sucess " + theme_name);
            }
        });
    }
